...
class GPT2Layer(nn.Module):
  ...
  def add(self, input, output, dense_layer, dropout):
    ...
    """
    ### YOUR CODE HERE
    # dense layer applied to output to change dim from intermediate size to hidden size
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    # Apply layer normalization and pass through the attention mechanism.
    attention = self.self_attention(self.attention_layer_norm(hidden_states), attention_mask)
    
    # Add residual connection (input + output of attention), apply dropout, and transform.
    attention_output = self.add(hidden_states, attention, self.attention_dense, self.attention_dropout)
    
    # Feed-forward network: apply layer norm, dense transformation, and activation.
    FFN_output = self.interm_af(self.interm_dense(self.out_layer_norm(attention_output)))
    
    # Add residual connection (input + output of FFN), apply dropout, and transform.
    output = self.add(attention_output, FFN_output, self.out_dense, self.out_dropout)
